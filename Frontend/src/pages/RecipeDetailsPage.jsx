...
const formatRecipeData = (meal) => {
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    ingredients.push({
      id: i,
      item: `${(measure || "").trim()} ${(ingredient || "").trim()}`.trim(),
    });
  }
  ...
  return {
    ...
    dietType: meal.strCategory,
    difficulty: "N/A", // API doesn't provide this, so we hardcode
    ingredients,
    steps,
...
export default function RecipeDetailsPage() {
  ...
  const [recipe, setRecipe] = useState(
    location.state?.meals ? formatRecipeData(location.state.meals[0]) : null
  );
   ...
   */
  useEffect(() => {
    // If we already have a recipe (from location.state), no need to fetch.
    if (recipe) {
      setLoading(false);
      return;
    }
    ...
    setLoading(true);
    axiosInstance
      .get(`/api/recipedetail/${id}/`)
      .then((res) => {
        if (res.data && res.data.length > 0) {
          // Format and set the recipe data
          setRecipe(formatRecipeData(res.data[0]));
        } else {
